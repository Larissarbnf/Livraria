...
public class BibliotecarioDAO implements Bibliotecario, Subject {
    ...
    public void registrarAluguel(AluguelDTO aluguel) throws SQLException {
        ...
        try (Connection con = ConexaoPostgres.getInstance().abrirConexao();
            ...
            pst.setLong(1, aluguel.getLivroId());
            pst.setLong(2, aluguel.getBibliotecarioId());
            pst.setDate(3, Date.valueOf(aluguel.getDataAluguel()));
            pst.setDate(4, Date.valueOf(aluguel.getDataDevolucaoPrevista()));
            pst.executeUpdate();
    ...
    @Override
    public List<RelatorioDTO> gerarRelatoriosAluguel() throws SQLException {
        List<RelatorioDTO> relatorios = new ArrayList<>();
        String sql = "SELECT livro_id, COUNT(*) AS quantidade_alugada FROM alugueis GROUP BY livro_id";
        try (Connection con = ConexaoPostgres.getInstance().abrirConexao();
             Statement stmt = con.createStatement();
            ...
            throw new SQLException("Erro ao gerar relatórios de aluguel. " + e.getMessage(), e);
        }
        return relatorios;
    }
}
