...
/*    @Override
    public List<RelatorioDTO> gerarRelatorioLivrosVendidos() throws SQLException {
        ...
        try (Connection con = ConexaoPostgres.getInstance().abrirConexao();
            ...
            while (rs.next()) {
                ...
                relatorio.setTotalAlugueis(rs.getInt("quantidade_alugada"));
    ...
    */
    ...
    public List<RelatorioDTO> gerarRelatorioLivrosVendidos() throws SQLException {
    	 List<RelatorioDTO> relatorios = new ArrayList<>();
    	    String sql = "SELECT l.titulo AS nome_livro, c.nome AS nome_cliente, v.data_venda " +
                    "FROM vendas v " +
                    "JOIN cliente c ON v.id_cliente = c.id " +
                    "JOIN livro l ON l.id = v.id_livro " +
    	    ...
    	    try (Connection con = ConexaoPostgres.getInstance().abrirConexao();
    	        ...
    	        while (rs.next()) {
    	            RelatorioDTO relatorio = new RelatorioDTO();
    	            relatorio.setNomeLivro(rs.getString("nome_livro"));
    	            relatorio.setNomeCliente(rs.getString("nome_cliente"));
    	            relatorio.setDataVenda(rs.getDate("data_venda")); 
    	            relatorios.add(relatorio);
    	        }
    	    } catch (SQLException e) {
    	        throw new SQLException("Erro ao gerar relatório de vendas. " + e.getMessage(), e);
    	    }
    	    return relatorios;
    	}
...
*/
    public List<RelatorioDTO> gerarRelatorioLivrosAlugados()throws SQLException {
        List<RelatorioDTO> relatorios = new ArrayList<>();
        String sql = "SELECT l.titulo AS nome_livro, c.nome AS nome_cliente, a.data_devolucao " +
                     "FROM alugueis a " +
                     "JOIN cliente c ON a.cliente_id = c.id " +
                     "JOIN livro l ON a.livro_id = l.id " +
        ...
        try (Connection con = ConexaoPostgres.getInstance().abrirConexao();
            ...
            while (rs.next()) {
                ...
                relatorio.setNomeLivro(rs.getString("nome_livro"));
                relatorio.setNomeCliente(rs.getString("nome_cliente"));
                relatorio.setDataDevolucao(rs.getDate("data_devolucao"));
                relatorios.add(relatorio);
            }
        } catch (SQLException e) {
            throw new SQLException("Erro ao gerar relatório de aluguéis. " + e.getMessage(), e);
        }
        return relatorios;
    }
